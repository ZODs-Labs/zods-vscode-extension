...
import CodeRefactoringModalButton from '../code/ChatCommandCodeModalButton';
...
const UserChatCommandBox = ({ chatCommand, lensName, code }: Props) => {
   const renderedBox = useMemo(() => {
      switch (chatCommand) {
         case AIChatCommand.AICodeLens:
            return (
               <CodeRefactoringModalButton
                  label={`AI Lens: ${lensName || ''}`}
                  code={code!}
                  icon={
                     <svg
                        width='22px'
   ...
   }, [chatCommand, code, lensName]);

   return renderedBox || <></>;
};

export default UserChatCommandBox;
